...
WinFontInfo *WinFontInfo::make(GString *nameA, GBool boldA, GBool italicA,
  ...
  int lc = mbstowcs( pwc, regName->getCString(), MB_CUR_MAX );

  if (RegQueryValueExW(regKey, pwc, NULL, NULL,
...
WinFontList::WinFontList(char *winFontDirA) {
  ...
  wchar_t *pwc      = (wchar_t *)malloc( sizeof( wchar_t ));
  int lc = mbstowcs( pwc, path, MB_CUR_MAX );

  if (RegOpenKeyExW(HKEY_LOCAL_MACHINE, pwc, 0,
...
GlobalParams::GlobalParams(char *cfgFileName) {
  ...
  if (!f) {
    ...
    char buf[512];
    i = GetModuleFileNameW(NULL, (LPWCH)buf, sizeof(buf));
    if (i <= 0 || i >= sizeof(buf)) {
