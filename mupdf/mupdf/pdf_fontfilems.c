...
lookupcidfont(pdf_fontdesc *font, int csi, int kind, char **fontpath, int *index)
{
	if (MINCHO == kind) {
		switch (csi) {
			...
			case Japan:
				if (font->flags & FD_FIXED) return lookupwindowsfont("MS-Mincho", fontpath, index);
				else return lookupwindowsfont("MS-PMincho", fontpath, index);
			case Korea:
	...
	if (GOTHIC == kind) {
		switch (csi) {
			...
			case Japan:
				if (font->flags & FD_FIXED) return lookupwindowsfont("MS-Gothic", fontpath, index);
				else return lookupwindowsfont("MS-PGothic", fontpath, index);
			case Korea:
